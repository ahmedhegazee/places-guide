...
function ($) {
    ...
    if (window.isMobile || window.isTouch) {
      ...
      switch (a) {
        ...
        default:
          ...
          if (_typeof(a) == "object" && typeof b == "undefined") {
  ...
  $.fn.calendar = function (opt, val) {
    ...
    var makeMonth = function makeMonth(y, m, d) {
      var body = $("<table>").addClass("month-body"); // .appendTo(month)
      ...
      var today = new Date();
      var todayDate = today.getDate();
      var thisMonth = !!(today.getMonth() + 1 === m && today.getFullYear() === y); // date.setFullYear(y,m-1,1);

      var numDays = new Date(y, m, 0).getDate();
      var numWeeks = Math.ceil((numDays + date.getDay()) / 7);
      var day = 1; // console.log(date.getDay(), date.getMonth(), numDays);
      ...
      for (var i = 0; i < 6; i++) {
        ...
        for (var j = 0; j < 7; j++) {
          if (i === 0 && j < date.getDay() || day > numDays) {
            wk.append("<td></td>"); // wk.append('<td class="fade"> - </td>')
            // continue;
          } else {
            $("<td>").text(day < 10 ? "0" + day : day) // .addClass(day === d ? 'active' : '')
            .addClass(thisMonth && day === todayDate ? "today" : "").attr("data-date", day) // .ripple({ type: 'icon' })
            .appendTo(wk).click(function (e) {
    ...
    var selectDate = function selectDate(y, m, d) {
      selected.d = d;
      selected.m = m;
      selected.y = y;
    ...
    var bringMonth = function bringMonth(p_n, y, m, d) {
      isMoving = true;
      ...
      def.toggle.bind(self)(current.y, current.m); // requestAnimationFrame(function() {
      ...
      body.one("transitionend", function (e) {
        ...
        month.removeClass("no-overflow"); // def.toggle.bind(self)(current.y, current.m);

        isMoving = false;
      });
    };

    var showYearPicker = function showYearPicker(e) {
      year.addClass("visible");
    };

    var hideYearPicker = function hideYearPicker(e) {
      year.removeClass("visible");
    };

    var isMoving = false;
    var date = def.date; // var month = $('<div>')

    var month = $(this[0]).addClass("nao-month"); // .appendTo(dpkr)

    var btn1 = $("<button>").addClass("ic ic-arrow-angle-left").ripple().attr("title", "Previous month").click(function (e) {
      e.preventDefault();
      if (isMoving) return;
    ...
    var btn2 = $("<button>").addClass("ic ic-arrow-angle-right").ripple().attr("title", "Next month").click(function (e) {
      e.preventDefault();
      if (isMoving) return;
    ...
    var btn3 = $("<button>").addClass("btn ic ic-arrow-circle-angle-down").ripple().attr("title", "View current month").click(function (e) {
      e.preventDefault();
      if (isMoving) return;
      var dt = new Date();
    ...
    var btn4 = $("<button>").addClass("btn ic ic-target").ripple().attr("title", "View selected date").click(function (e) {
      e.preventDefault();
      if (isMoving) return;
    ...
    var yMon = $("<div>").text(yearMonths[date.getMonth()] + " " + date.getFullYear()).attr("title", "Select year and month").click(showYearPicker);
    var head = $("<div>").addClass("month-head") // .text(yearMonths[date.getMonth()]+' '+date.getFullYear())
    .append(btn1).append(btn3).append(btn2).append(btn4).append(yMon).appendTo(month);
    ...
    var yrnx = $("<button>") // .addClass('span')
    .addClass("ic ic-arrow-angle-right").ripple().click(function (e) {
      e.preventDefault();
      if (isMoving) return;
      current.y++;
      yrnm.text(current.y);
    ...
    var yrpv = $("<button>") // .addClass('span')
    .addClass("ic ic-arrow-angle-left").ripple().click(function (e) {
      e.preventDefault();
      if (isMoving) return;
      current.y--;
      yrnm.text(current.y);
    ...
    var yrnm = $("<div>").text(current.y).attr("title", "Go back to calendar").click(hideYearPicker);
    var yrhd = $("<div>").addClass("month-head") // .addClass('year-head')
    .append(yrpv).append(yrnx).append(yrnm).appendTo(year);
    var yrbd = $("<table>").addClass("year-body").appendTo(year).html("<tbody></tbody>").find("tbody");
    var yrft = $("<div>").addClass("year-footer ic ic-arrow-circle-angle-up") // .text('okay')
    .attr("title", "Go back to calendar").ripple().appendTo(year).click(function (e) {
      year.removeClass("visible");
    });
    ...
    for (var i = 0; i < 4; i++) {
      ...
      for (var j = 0; j < 3; j++) {
        var idx = i * 3 + j;
        var col = $("<td>").text(yearMonths[idx]).attr("data-month", idx + 1).addClass(current.m === idx + 1 ? "active" : "").appendTo(row).ripple().click(function (e) {
          if (isMoving) return;
